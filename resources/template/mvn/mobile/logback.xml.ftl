...
<configuration>
  ...
  <appender name="FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
    <file>${r"$"}{log.basedir}/${javaProjectName}-mobile.log</file>
    <rollingPolicy class="ch.qos.logback.core.rolling.TimeBasedRollingPolicy">
      <fileNamePattern>${r"$"}{log.basedir}/${javaProjectName}-mobile.log.%d{yyyy-MM-dd}</fileNamePattern>
    </rollingPolicy>
    <encoder>
      <pattern>${r"$"}{default_pattern}</pattern>
      <charset>${r"$"}{charset}</charset>
  ...
  <logger name="${basePackageName}.${javaProjectName}">
