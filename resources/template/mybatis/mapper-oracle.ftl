...
<mapper namespace="${modelClassSimpleName?uncap_first}DAO">
  ...
  <sql id="sql_insert_property_list">
    ...
    <#else>
      ...
      <#if propertyName == "createdAt" || propertyName == "updatedAt">
      <if test="entity.${propertyName} == null">,SYSDATE</if>
      </#if>
    </#if>
    </#list>
    </trim>
      ...
      <#else>
        <choose>
          ...
          <otherwise>
            ...
            <if test="entity.${propertyName} == null">,${columnInfoList[propertyName_index].columnName} = SYSDATE</if>
            </#if>
          </otherwise>
        </choose>
      </#if>
  ...
  <![CDATA[
    ...
    FROM (SELECT rid, rnum FROM (SELECT ROWID AS rid, ROWNUM AS rnum FROM (
  ]]>
  </sql>

  <sql id="sql_pagination_end">
  <![CDATA[
    ) WHERE ROWNUM <= ${r"#"}{endRow}) WHERE rnum >= ${r"#"}{startRow}) a, ${tableName} t WHERE a.rid = t.ROWID
  ]]>
  </sql>

  <insert id="insert" parameterType="java.util.Map">
    <#if pkType == "java.lang.Integer" || pkType == "java.lang.Long">
    <selectKey keyProperty="entity.id" resultType="${pkType}" order="BEFORE">
